...
const SearchField = () => {
  ...
  // useEffect(() => {
  //   if (bikes?.data && bikes) {
  //     const getUniqueOptions = <T, K extends keyof T>(data: T[], key: K, allLabel: string): TSelectType[] => {
  //       const uniqueOptions = data.reduce((acc: TSelectType[], item: T) => {
  //         if (!acc.some(option => option.value === item[key])) {
  ...
  //     // Set brands, models, and years
  //     setBrands(getUniqueOptions(bikes.data, "brand", "All Brands"));
  //     setModels(getUniqueOptions(bikes.data, "model", "All Models"));
  //     setYears(getUniqueOptions(bikes.data, "year", "All Years"));
  //   }
  // }, [bikes]);
